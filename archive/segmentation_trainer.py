...
from torch_geometric.datasets import ShapeNet, S3DIS
...
category = 'Guitar' # Pass in `None` to train on all categories.
path = osp.join(osp.dirname(osp.realpath(__file__)), '..', '..', 'S3DIS')
transform = T.Compose([
...
train_dataset = S3DIS(path, 6, True)
test_dataset = S3DIS(path, 6, False)
train_loader = DataLoader(train_dataset, batch_size=15, num_workers=6, shuffle=True)
test_loader = DataLoader(test_dataset, batch_size=15, num_workers=6, shuffle=False)

path = osp.join(osp.dirname(osp.realpath(__file__)), '..', '..', 'ShapeNet')

train_dataset2 = ShapeNet(path, category, split='trainval', transform=transform,
                         pre_transform=pre_transform)
test_dataset2 = ShapeNet(path, category, split='test',
                        pre_transform=pre_transform)
train_loader2 = DataLoader(train_dataset2, batch_size=10, shuffle=True,
                          num_workers=6)
test_loader2 = DataLoader(test_dataset2, batch_size=10, shuffle=False,
                         num_workers=6)
